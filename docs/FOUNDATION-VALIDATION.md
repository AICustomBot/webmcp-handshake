...
| --- | --- | --- |
| Workspace inventory | `find . -type f \| wc -l` | 42 files present and synced |
| JSON parse | `json.loads` across all `.json` | Pass |
...
| License | `grep` on `LICENSE` | Apache License 2.0 confirmed |
| Credential scan | recursive regex for key/secret/password/token/private-key assignments | No hardcoded credential patterns |
| Static asset shell | headless Chromium capture of `apps/web/public/index.html` | No console errors, exceptions, failed resources, or overflow clipping |
